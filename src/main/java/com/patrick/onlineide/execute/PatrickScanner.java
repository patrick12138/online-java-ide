package com.patrick.onlineide.execute;
 ...
 * PatrickScanner(InputStream source)
 * <p>
 * 然后在把所有报错的地方修改一下，主要修改：
 * 1. 构造函数的类名：Scanner -> PatrickScanner
 * 2. 几个方法的返回参数类型：java.util.Scanner -> org.olexec.execute.PatrickScanner
 * <p>
 * PS. Scanner 类要是能 extends 就好了，这样就不用写这么多了，现在这个类看起来好吓人...
 */
public final class PatrickScanner implements Iterator<String>, Closeable {
     ...
     */
    private PatrickScanner(Readable source, Pattern pattern) {
     ...
     *               interface
     */
    public PatrickScanner(Readable source) {
     ...
     */
    public PatrickScanner(InputStream source) {
        // 判断一下输入的 InputStream 是不是 PatrickInputStream，
        // 如果是就调用一下它的 get 方法，把当前线程的输入流从 ThreadLocal 中取出来传入
        this(new InputStreamReader(
                        (source instanceof PatrickInputStream) ? ((PatrickInputStream) source).get() : source),
                WHITESPACE_PATTERN);
    }
     ...
     *                                  does not exist
     */
    public PatrickScanner(InputStream source, String charsetName) {
     ...
     */
    public PatrickScanner(File source) throws FileNotFoundException {
     ...
     *                                  not found
     */
    public PatrickScanner(File source, String charsetName)
            throws FileNotFoundException {
        this(Objects.requireNonNull(source), toDecoder(charsetName));
    }

    private PatrickScanner(File source, CharsetDecoder dec)
            throws FileNotFoundException {
     ...
     * @since 1.7
     */
    public PatrickScanner(Path source)
            throws IOException {
     ...
     * @since 1.7
     */
    public PatrickScanner(Path source, String charsetName) throws IOException {
        this(Objects.requireNonNull(source), toCharset(charsetName));
    }

    private PatrickScanner(Path source, Charset charset) throws IOException {
     ...
     */
    public PatrickScanner(String source) {
     ...
     */
    public PatrickScanner(ReadableByteChannel source) {
     ...
     *                                  does not exist
     */
    public PatrickScanner(ReadableByteChannel source, String charsetName) {
     ...
     * @return this scanner
     */
    public PatrickScanner useDelimiter(Pattern pattern) {
        delimPattern = pattern;
        return this;
    }
     ...
     * @return this scanner
     */
    public PatrickScanner useDelimiter(String pattern) {
     ...
     * @return this scanner
     */
    public PatrickScanner useLocale(Locale locale) {
     ...
     */
    public PatrickScanner useRadix(int radix) {
     ...
     * @throws IllegalStateException  if this scanner is closed
     */
    public PatrickScanner skip(Pattern pattern) {
        ensureOpen();
        if (pattern == null)
            throw new NullPointerException();
        clearCaches();
     ...
     * @throws IllegalStateException if this scanner is closed
     */
    public PatrickScanner skip(String pattern) {
