 ...
 *    java/lang/System -> com/jvm/ch9/remoteinvoke/PatrickSystem
...
//    private static volatile AtomicInteger runningCount = new AtomicInteger(0);
    ...
    public static String execute(byte[] classByte, String systemIn) {
        ...
        // 3. 调用ClassModifier#modifyUTF8Constant修改
        byte[] modifyBytes = cm.modifyUTF8Constant("java/lang/System", "org/olexec/execute/PatrickSystem");
        modifyBytes = cm.modifyUTF8Constant("java/util/Scanner", "org/olexec/execute/PatrickScanner");

        // 设置用户传入的标准输入
        ((PatrickInputStream) PatrickSystem.in).set(systemIn);
        ...
        try {
            ...
            而是要把异常信息 print 到 PatrickSystem.err 以反馈给客户端
            */
            e.getCause().printStackTrace(PatrickSystem.err);
        }

        // 6. 从HackSystem中获取返回结果
        String res = PatrickSystem.getBufferString();
        PatrickSystem.closeBuffer();
        return res;
    }
}
